...
from assumptions import (
    load_property,
    PRIMARY_APPRECIATION, APPRECIATION, APPRECIATION_NOTES, APPRECIATION_TAGS,
    HORIZONS, WORKED_EXAMPLE_HORIZON, INVEST_RATES, RENT_GROWTH,
    MARGINAL_TAX, CAP_GAINS_RATE, DEPREC_RECAPTURE_RATE, SALE_COST_RATE,
    BROKER_RATE, TRANSFER_TAX, TITLE_ESCROW, FED_LT_CAP_GAINS, NIIT_RATE, CA_TOP_RATE,
    FED_RECAPTURE, AFTERTAX_OPP, PRIMARY_INVEST,
    BAD_VACANCY_MONTHS, EVICTION_COST, MAJOR_REPAIR, RISK_VACANCY_PROB, RISK_EVICTION_PROB,
    RISK_REPAIR_PROB, MOVE_BACK_YEARS, VACANCY_RATE, MGMT_RATE, TENANCY_YEARS,
    LEASING_FEE_MONTHS, CG_EXCLUSION, PASSIVE_LOSS_MAGI_LIMIT, INCOME_BRACKET_THRESHOLD,
    MONTHS_PER_YEAR,
)
...
def build_context(m: Model) -> dict:
    ...
    for rent in p.realistic_rents:
        ...
        headline += _nw_row(f'<b>Hold, rent ${rent/1000:g}k/mo</b> '
                            f'<span class="sub">(full rental)</span>', vals,
                            cls="primary", bold_last=True)
    w3 = ""
    for y in H:
        if y <= 3:
            v = m.hold_net_worth(p.primary_rent, y, PRIMARY_APPRECIATION, sec121="within_3yr").net_worth
            w3 += f"<td>{v:,.0f}</td>"
        else:
            w3 += '<td class="sub">n/a*</td>'
    headline += (f'<tr><td>Hold ≤3 yrs then sell '
                 f'<span class="sub">(keeps full ${CG_EXCLUSION/1000:g}k §121)</span></td>{w3}</tr>')
    mb = [m.hold_net_worth(p.primary_rent, y, PRIMARY_APPRECIATION, sec121="move_back").net_worth for y in H]
    headline += _nw_row(f'Hold, rent ${p.primary_rent/1000:g}k/mo '
                        f'<span class="sub">(move back {MOVE_BACK_YEARS} yrs, partial §121)</span>', mb)
    for rate in INVEST_RATES:
        vals = [m.invest_net_worth(sell.net_proceeds, y, rate) for y in H]
        headline += _nw_row(f'<b>Sell now + invest @ {rate*100:.0f}%</b> '
                            f'<span class="sub">(after-tax)</span>', vals, cls="sell")
    ...
    for key, appr in APPRECIATION.items():
        ...
        lbl = (f'{appr*100:g}%/yr <span class="pill {pill_by_key[key]}">{APPRECIATION_TAGS[key]}</span> '
    ...
    # Out-of-pocket cash table
    def oop_row(idx):
        cells = ""
        for r in p.realistic_rents:
            v = m.oop_breakdown(r)[idx]
    ...
    oop_rows = [oop_row(i) for i in range(4)]   # 4 cash-item rows; index 4 = net
    oop_net = "".join(f'<td class="num-bad">−{abs(m.oop_breakdown(r)[4]):,.0f}</td>' for r in p.realistic_rents)
    oop_mo = "".join(f'<td class="num-bad">−{abs(m.oop_breakdown(r)[4]/MONTHS_PER_YEAR):,.0f}</td>' for r in p.realistic_rents)

    inc_lo = sell.net_proceeds * INVEST_RATES[0]
    inc_hi = sell.net_proceeds * INVEST_RATES[1]

    # Risk rows
    base_oop = m.oop_breakdown(p.primary_rent)[4]
    scenarios = [
        ("Normal year (baseline)", 0, base_oop),
        (f"+ {BAD_VACANCY_MONTHS} months extra vacancy", -BAD_VACANCY_MONTHS*p.primary_rent, base_oop - BAD_VACANCY_MONTHS*p.primary_rent),
    ...
    risk_rows += (f'<tr class="total"><td>WORST CASE: all three at once</td>'
    ...
    a = [
        ...
        ("Mortgage rate", f"{m.apr*100:.3f}%", f"derived from ${p.monthly_pi:,.2f} P&amp;I + {p.payments_left} payments"),
        ...
        ("Rent (realistic)", f"${p.realistic_rents[0]:,.0f}–{p.realistic_rents[-1]:,.0f}/mo", "local comps"),
        ...
        ("Property management", f"{MGMT_RATE*100:g}% + {LEASING_FEE_MONTHS:g}-mo leasing", f"leasing fee amortized over {TENANCY_YEARS:g}-yr tenancy"),
        ("Sale costs", f"{SALE_COST_RATE*100:.2f}%", f"{BROKER_RATE*100:g}% broker + {TRANSFER_TAX*100:g}% transfer + {TITLE_ESCROW*100:g}% title"),
        ("Marginal / ordinary tax", f"{MARGINAL_TAX*100:g}%", f"single, &gt;${INCOME_BRACKET_THRESHOLD/1000:g}k income"),
        ("Cap-gains rate (future)", f"{CAP_GAINS_RATE*100:.1f}%", f"fed {FED_LT_CAP_GAINS*100:g}% + NIIT {NIIT_RATE*100:g}% + CA {CA_TOP_RATE*100:g}%"),
        ("Depreciation recapture", f"{DEPREC_RECAPTURE_RATE*100:.1f}%", f"fed {FED_RECAPTURE*100:g}% + CA ordinary {CA_TOP_RATE*100:g}%"),
        ("Passive losses", "suspended", f"MAGI &gt;${PASSIVE_LOSS_MAGI_LIMIT/1000:g}k → released at sale"),
        ("Investment return (sell)", f"{INVEST_RATES[0]*100:g}% / {INVEST_RATES[1]*100:g}% pre-tax", "conservative / S&amp;P long-run nominal"),
        ("Hold opportunity cost", f"{AFTERTAX_OPP*100:.1f}% after-tax", f"{PRIMARY_INVEST*100:g}% × (1−{CAP_GAINS_RATE*100:.1f}% cap-gains); on neg. cash flow &amp; reserve"),
        ("Expected risk drag", f"${m.expected_risk_drag:,.0f}/yr", f"prob-weighted: {RISK_VACANCY_PROB*100:g}%×{m.excess_vacancy_months:.1f}mo excess vacancy + {RISK_EVICTION_PROB*100:g}%×eviction + {RISK_REPAIR_PROB*100:g}%×repair"),
        ("Bad-year events", "vac/evict/repair", f"{BAD_VACANCY_MONTHS}mo vacancy, ${EVICTION_COST:,.0f} eviction, ${MAJOR_REPAIR:,.0f} repair (stacked)"),
        ("§121 move-back proration", "qualified/total yrs", f"(res {p.years_owned_as_residence:g}+{MOVE_BACK_YEARS}) / (res {p.years_owned_as_residence:g}+rental+{MOVE_BACK_YEARS}); approximate"),
    ...
    if v["win_cells"] == v["total_cells"]:
        ...
        headline_verdict = ("selling comes out ahead at the central case across the realistic rents once "
                            "future sale taxes and costs are counted")
    else:
        headline_verdict = (f"the two are close at the central case — holding wins in {v['win_cells']} of "
                            f"{v['total_cells']} realistic rent/horizon combinations, and the pessimistic "
                            f"{low_pct:g}% case it {v['verb_low_long']} selling")

    take = (f"At the central {mod_pct:g}% assumption, holding {v['verb_10']} selling at 10 years "
    ...
    bet_framing = (
        ...
        + ("The downside loss is larger than the central-case gain — "
           if abs(v['downside']) > abs(v['central_edge']) else "")
    ...
    M = Markup
    return {
        "address": p.address, "generated": p.as_of_date,
        "home_value": p.home_value, "cost_basis": p.cost_basis, "mortgage_bal": p.mortgage_bal,
        "apr": m.apr, "primary_invest": PRIMARY_INVEST, "aftertax_opp": AFTERTAX_OPP,
        "marginal_tax": MARGINAL_TAX, "cap_gains_rate": CAP_GAINS_RATE,
        "deprec_recapture_rate": DEPREC_RECAPTURE_RATE, "annual_depreciation": m.annual_depreciation,
        "cash_reserve": p.cash_reserve, "rent_growth": RENT_GROWTH,
        "primary_rent": p.primary_rent, "cg_exclusion": CG_EXCLUSION,
        "sale_cost_rate": SALE_COST_RATE, "worked_horizon": WORKED_EXAMPLE_HORIZON,
        "primary_appreciation": PRIMARY_APPRECIATION, "passive_loss_magi_limit": PASSIVE_LOSS_MAGI_LIMIT,
        "sell": sell, "we": we,
        "deprec_net": f"{we.deprec_release - we.recapture:+,.0f}",
        "hz_head": M(hz_head), "headline_rows": M(headline), "appr_rows": M(appr_rows),
        "oop_head": M(oop_head), "oop_rows": [M(x) for x in oop_rows],
        "oop_net": M(oop_net), "oop_mo": M(oop_mo),
        "risk_rows": M(risk_rows), "assumption_rows": M(assumption_rows),
        "inc_lo": inc_lo, "inc_hi": inc_hi, "base_oop": base_oop, "worst_total": worst_total,
        "mo_oop": v["mo_oop"], "yr1_oop": v["yr1_oop"], "yr10_oop": v["yr10_oop"],
        "cum_oop_10": v["cum_oop_10"], "reserve_cost_yr": v["reserve_cost_yr"],
        "verb_low_long": v["verb_low_long"],
        "headline_verdict": M(headline_verdict), "take": M(take), "bet_framing": M(bet_framing),
    }
...
def write_text_summary(mdl, ctx, path):
    ...
    p = mdl.p
    sell = ctx["sell"]
    def fmt(n): return f"${n:>13,.0f}"
    lines = []
    lines.append("=" * 70)
    lines.append(f"  RENT vs. SELL — {p.address}  (generated {p.as_of_date})")
    lines.append("=" * 70)
    lines.append(f"  Value {fmt(p.home_value).strip()} | Basis {fmt(p.cost_basis).strip()} | "
    ...
    hold_full = [mdl.hold_net_worth(p.primary_rent, y, PRIMARY_APPRECIATION).net_worth for y in HORIZONS]
    lines.append("  " + "Hold (full rental)".ljust(26) + "".join(fmt(x).rjust(13) for x in hold_full))
    for rate in INVEST_RATES:
        row = [mdl.invest_net_worth(sell.net_proceeds, y, rate) for y in HORIZONS]
        lines.append("  " + f"Sell + invest @ {rate*100:.0f}%".ljust(26) + "".join(fmt(x).rjust(13) for x in row))
    we = ctx["we"]
...
def main(property_path: str = DEFAULT_PROPERTY):
    ...
    print(f"[wrote {OUTDIR}/report.html, {OUTDIR}/interpretation.html, {OUTDIR}/results.txt for {property_path}]")
